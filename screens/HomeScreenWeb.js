...
const HomeScreen = () => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <View style={{ padding: 16, maxWidth: isWeb ? 1400 : width, alignSelf: 'center', flex: 1 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <Bars3CenterLeftIcon size={30} color={storeColors.textBlack} />
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ fontSize: 24, fontWeight: 'bold', color: storeColors.textBlack }}>Smart Home</Text>
            <ArrowDownCircleIcon size={18} color={storeColors.textBlack} />
          </View>
          {isWeb && (
            <TouchableOpacity onPress={() => Alert.alert('Go to Profile Screen')}>
              <Image source={require('../assets/images/avatar.jpg')} style={{ width: 50, height: 50, borderRadius: 25 }} />
            </TouchableOpacity>
          )}
        </View>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
          <View style={{ marginTop: 16, marginHorizontal: 16 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 20 }}>
              <ModeApp icon={<LightBulbIcon size={18} color={storeColors.textBlack} />} text="Light mode" onPress={() => Alert.alert('Chế độ sáng')} />
              <ModeApp icon={<LightBulbIcon size={18} color={storeColors.textBlack} />} text="Dark mode" onPress={() => Alert.alert('Chế độ tối')} />
            </View>

            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 20 }}>
              <ModeApp icon={<ArrowUpCircleIcon size={24} color={storeColors.textBlack} />} text="Mở cửa nhà" onPress={openDoor} />
              <ModeApp icon={<ArrowDownCircleIcon size={24} color={storeColors.textBlack} />} text="Đóng cửa nhà" onPress={closeDoor} />
            </View>
          ...
          <View style={{ marginTop: 40, alignItems: 'center' }}>
            <Text style={{ fontSize: 16, color: '#666' }}>Cre Việt Hoàng</Text>
          </View>
